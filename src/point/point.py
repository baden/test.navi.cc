...
def add_point(time, lat, lon, ftype):
    lat_s = 0
    if lat < 0:
        lat_s = 1
        lat = -lat
    ...
    packet = pack(PACK_F2,
        ...
        ftype,     # D27   Тип точки   Причина фиксации точки
...
points = json.loads(open("p0.txt").read())

print("Points=", len(points))
print("Point1=", repr(points[0]))
...
#packet = randint(1, 20)     # Размер пакета

body = ''

for p in points:
    print("Point: %s" % (str(now)))
    #if randint(1, 30) == 1:     # Примерно каждая 30я точка - точка остановки
    #ftype =
    body += add_point(now, p[0], p[1], FTYPES["ANGLE"])
